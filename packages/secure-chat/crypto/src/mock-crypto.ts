...
//
// Relicensed to Apache-2.0 by the sole author on moving it from the (AGPL) agora-server repo into
// this SDK package — its true home, since the seam is client crypto. Exported only from the
// `@agora-sdk/secure-chat-crypto/testing` subpath so nobody mistakes it for real crypto.
//
// It is NOT cryptographically secure — it is the test double that lets the server's MLS Delivery
// Service be exercised end-to-end WITHOUT a real MLS library, and a placeholder the web client can
// wire against before the concrete core lands. Two properties make it useful:
//
//  1. **Reversible + plaintext-hiding.** `encryptMessage` XORs the plaintext with a non-zero
//     keystream derived from a per-group secret, so the wrapped bytes never contain the plaintext —
//     the server-blindness assertion in the integration tests is meaningful.
//  2. **Cross-instance.** The group secret travels inside the Welcome, so a *different* mock
//     instance (a second "device") can `processWelcome` and then decrypt the first one's messages —
//     modelling real two-party delivery.
import type {
  SecureChatCrypto,
  DeviceIdentity,
  KeyPackageBundle,
